...
As fine-tuning requires training data and hardware resources, DB-BERT then uses BART \cite{DBLP:journals/corr/abs-1910-13461} as its language model (despite the project name) to leverage its zero-shot classification ability, and thus no longer requires training.
