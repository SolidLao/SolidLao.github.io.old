...
Since fine-tuning requires training data and hardware resources, DB-BERT then uses BART as its language model (despite the project name) to leverage its zero-shot classification ability, and thus no longer requires training.
